...
//    }
    ...
    public static void updateEvent(Event event) {
        event.setUpdatedAt(new Date());
        db.collection(collection).document(event.getReference().getId()).set(event);
    }
    ...
    public static Event GetEvent(DocumentReference evId) {
        ...
        db.collection(collection).document(evId.getId()).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
    ...
    public DocumentReference getReference() {
        // get the reference to the event in the database
